...
export default class RecentAnalystActivity extends Component {
    ...
    handleClick(id){
        window.open(`/report-detail/article/${id}`);
    }
    render(){
        const _this = this;
                    ...
                    <div className="itemListWrap">
                        {
                            new_list.map( (item,index) => (
                                ...
                                <div className="item" key={index}>
                                    <p className="itemDetail"><em className="icon_clock"><Icon type="clock-circle-o" /></em><a target="_blank" title={item.title} onClick={()=>{this.handleClick(item.id)}} >{item.title}</a></p>
                                    <p className="itemDate">{timeFromNow === 'o' ? item.time : this.dateFormat(item.time, timeFromNow) }</p>
                                    <p className="itemSource" onClick={(e) =>subTitClick({index,item},e)}>{item.source}</p>
                                </div> :
                                <div className="item" key={index}>
                                    <p className="itemDetail"><em className="snaps"></em><a href={item.href} target="_blank" title={item.title} onClick={()=>{this.handleClick(item.id)}} >{item.title}</a></p>
                                    <p className="itemDate">{timeFromNow === 'o' ? item.time : this.dateFormat(item.time, timeFromNow) }</p>
                                    <p className="itemSource" onClick={(e) =>subTitClick({index,item},e)}>{item.source}</p>
                                </div>
                            ) )
...
RecentAnalystActivity.defaultProps = {
    title : '',
    list : [],
    titleClick(){},
    subTitClick(){},
