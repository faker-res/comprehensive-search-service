...
// Autosuggest组件主题类名映射
const threeLevelSearchBoxTheme = {
  container: 'three-level-search-box__container',
  containerOpen: 'three-level-search-box__container--open',
  input: 'three-level-search-box__input',
  inputOpen: 'three-level-search-box__input--open',
  inputFocused: 'three-level-search-box__input--focused',
  suggestionsContainer: 'three-level-search-box__suggestions-container',
  suggestionsContainerOpen: 'three-level-search-box__suggestions-container--open',
  suggestionsList: 'three-level-search-box__suggestions-list',
  suggestion: 'three-level-search-box__suggestion',
  suggestionFirst: 'three-level-search-box__suggestion--first',
  suggestionHighlighted: 'three-level-search-box__suggestion--highlighted',
  sectionContainer: 'three-level-search-box__section-container',
  sectionContainerFirst: 'three-level-search-box__section-container--first',
  sectionTitle: 'three-level-search-box__section-title'
};
...
// @inject('suggestStore')
// @translate()
export default class ThreeLevelSearchBox extends Component {
  // 默认属性
  static defaultProps = {
    // 搜索框占位文本
    placeholder: '',
    ...
    searchHistoryKey: 'SEARCH_THREELEVEL_HISTORY',
  ...
  static propTypes = {
    ...
    showClearBtn: PropTypes.bool,
     ...
     // 点击清除输入框按钮回调
     clickClearBtnCallBack:PropTypes.func,
  ...
  componentDidMount() {
    ...
    document.querySelector('.three-level-search-box')
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      || prevState.searchHistory !== this.state.searchHistory
    ) {
      // 更新建议列表
      this.setState({ suggestList: this.getCurrentSuggestList() });
    }
  ...
  handleInputBlur = (e) => {
    ...
    if (e.relatedTarget && e.relatedTarget.classList.contains('three-level-search-box__btn-empty')) {
      return false;
    }
  ...
  emptyKeyword = () => {
    ...
    this.props.clickClearBtnCallBack&&this.props.clickClearBtnCallBack();

  }

  //触发搜索的来源
  searchSourceFrom = ({ suggestion, suggestionValue, location, method }) => {
    // console.log('searchSourceFromFun');
    let input_from, page = 'threeLevel_search';
    if (!suggestion) {
      input_from = 'direct';
    } else {
  ...
  handleSearch = (value) => {
    ...
    if (_.isFunction(this.props.onSearch)) {
      this.props.onSearch({ keyword: query.keyword });
    } else {
      // 更新路由
  ...
  handleRequestSearchSuggest = ({ value }) => {
    ...
    // console.log('handleRequestSearchSuggestFun');
  ...
  // 请求清除建议列表处理
  handleRequestClearSuggestList = (value) => {
    // console.log('handleRequestClearSuggestListFun');
    this.setState({ suggestList: [] });
  }

  // 获取建议列表分组的建议字段数据
  getSectionSuggestions = (section) => {
    // console.log('getSectionSuggestionsFun=>',section);
    return section.suggestions;
  }

  // 获取建议列表项的建议值字段数据
  getSuggestionValue = (suggestion) => {
    // console.log('getSuggestionValueFun=>',suggestion);
  ...
  getNormalizedSearchHistory(keyword = this.state.keyword) {
    ...
    return history.length > 0 ? ThreeLevelSearchBox.getNormalizedSuggestionData({
  ...
  renderSectionTitle = (section, { isSearchHistory } = {}) => {
    // console.log('renderSectionTitleFun=>',section);
    const { t } = this.props;
      ...
      <div key={section.type}>
        ...
        {isSearchHistory &&
          <Icon
            ...
            className="three-level-search-box__action-btn three-level-search-box__btn-empty-history"
            onMouseDown={this.prevntSearchInputBlur}
            onClick={this.handleEmptySearchHistory}
            title={'清空'}
            // title={t('threeLevelSearchBox.emptyHistory', { defaultValue: '清空' })}
          />
        }
      </div>
    );
      ...
      <div key={suggestion.id}>
        ...
        {isSearchHistory &&
          <Icon
            ...
            className="three-level-search-box__action-btn three-level-search-box__btn-remove-history-item"
            onMouseDown={this.prevntSearchInputBlur}
            onClick={e => this.handleRemoveHistroyItem(e, suggestion.originSuggestion)}
            // title={t('threeLevelSearchBox.removeHistoryItem', { defaultValue: '删除' })}
            title={'删除'}
          />
        }
      </div>
  ...
  render() {
    const { t, style, className, placeholder, location, showClearBtn } = this.props;
    ...
    // 后缀按钮（清空按钮）渲染逻辑
    const suffix = keyword && showClearBtn
      ? <Icon
        type="close-circle"
        key="btn-empty"
        tabIndex="0"
        className="three-level-search-box__icon-suffix three-level-search-box__btn-empty"
    ...
    const inputProps = {
      ...
      placeholder,
      value: keyword,
      prefix: <Icon type="search" className="three-level-search-box__icon-prefix" />,
      // enterButton: t('threeLevelSearchBox.searchButtonText', { defaultValue: '搜索' }),
      enterButton: '搜索',
    ...
    return (
      <div style={style} className={searchBoxClass}>
        <Autosuggest
          theme={threeLevelSearchBoxTheme}
          multiSection={true}
