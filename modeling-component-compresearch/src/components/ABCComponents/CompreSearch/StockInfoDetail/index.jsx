...
// @translate()
export default class StockInfoDetail extends Component {
	constructor(props){
		super();
			...
			<div className={localClass}>
				<div className={parseFloat(zdf) > 0 ? 'up' : parseFloat(zdf) < 0 ? 'down' : ''}>
					<Row className="row" gutter={16}>
						...
						<Col className="col" span={10}><span className="dataTit">{"最新价"}</span><i className="dataNum">{price}</i></Col>
					</Row>
					<Row className="row" gutter={16}>
						<Col className="col" span={14}><div className="date">{date} | {count}</div></Col>
						<Col className="col" span={10}><span className="dataTit">{"涨跌幅"}</span><i className="dataNum">{zdp} / {zdf_p}%</i></Col>
					</Row>
				</div>
			</div>
		)
