...
@withRouter
@inject(Store => ({
  honor: Store.honor.honorState,
  fetchHonorDetail: Store.honor.fetchHonorDetail,
}))
@observer
export default class Honor extends Component {
  getContent = () => {
    const { honorDetail = {} } = this.props.honor;
    return (
      <div className={'honor_content'}>
        ...
        {
          (honorDetail.honor || []).map((honor, index) => {
            return (
              <div key={index} className={`middle honor_type`}>
