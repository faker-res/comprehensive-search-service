            ...
            <FullScreen isShow={isShow}>
                <div className={`abc-fullscrren-chart ${className}`}>
                    <div className={'header-bar'}>
                        <div className={'title'}>
                            <span>{this.removeDomOfText(config.image_title || '')}</span>
                        </div>
                                ...
                                <Menu
                                    onSelect={(item) => {
                                        if (item.key == 'excel') {
                                            FileExport.export({
                                                type: 'excel',
                                                data: FileExport.transformer(config.chart_data).data || [],
                                                fname: config.image_title
                                            });
                                        } else {
                                            utils.downloadImg(`/search-api/download/image?title=${window.encodeURIComponent(config.image_title)}&url=${window.encodeURIComponent(config.url || config.image_url)}`, config.image_title);
                                        }
                                    }}
                                    selectedKeys={['']}
                                    mode="horizontal">
                                    ...
                                    <Menu.SubMenu title={(
                                        ...
                                        {config.chart_data && !!config.chart_data.trim() && <Menu.Item key="excel">导出数据</Menu.Item>}
