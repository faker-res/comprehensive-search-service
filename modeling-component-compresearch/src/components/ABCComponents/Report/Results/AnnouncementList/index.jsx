...
class AnnouncementList extends Component {
    ...
    loadData(reload, {keyword, filters, order, industry, report, company, selected, start_time, end_time}) {
        ...
        // let method = this.props.location.pathname === '/report/secret/report-results' ? 'PrivateReportSearch' : 'VenderReportSearch';
        // if (typeof loadInternalData != 'undefined') {
        //     method = loadInternalData ? 'PrivateReportSearch' : 'VenderReportSearch';
        // }
            ...
            <div ref={this.handleRef} className={`abc-report-search-results ${className}`}>
                {
                    resultList.map((item, index) => {
                        return <AnnouncementItem key={index} data={item} highlight={nQueryParams} 
                            onClick={(data = {}) => { 
                                window.open(`${origin_notice}/notice/detail/html/?srcId=${data.srcId}`);
                            }
                        }/>;
                    })
                }
